...
> ⚠️ **Scope mismatch, flagged 2026-07-27:** the target product is a **general, multi-brand electronics warranty tracker** (any category, any manufacturer), not a single-company solar catalog. The codebase this document describes has been narrowed to one brand ("JP Solar") and solar-only categories via the `revamp_solar_products.sql` migration, and has **no `brands` table at all** — `Product` has no brand relationship. Treat everything below as documentation of *this one implementation*, not as the intended requirements. See [Requirements_Guide.md](Requirements_Guide.md) for the general, brand-agnostic requirements, and [Beginner_Build_Guide.md](Beginner_Build_Guide.md) for a build path that includes a `Brand` entity. Reconciling the code to the general model (adding `Brand`, generalizing categories/seed data, removing JP-Solar-specific copy) is out of scope for this pass — docs only, per explicit decision.
